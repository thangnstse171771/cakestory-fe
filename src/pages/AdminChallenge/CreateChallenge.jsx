...
const CreateChallengeSchema = Yup.object().shape({
  title: Yup.string().required("Hãy nhập tên sự kiện.").max(100),
  description: Yup.string().required("Hãy nhập mô tả.").max(1000),
  start_date: Yup.date()
  ...
  prize_description: Yup.string().required("Hãy nhập giải thưởng."),
  ...
  rules: Yup.string().required("Hãy nhập luật chơi."),
  requirements: Yup.string().required("Hãy nhập yêu cầu."),
  avatar: Yup.string().required("Hãy chọn ảnh đại diện."),
});

const CreateChallenge = ({ isOpen, onClose, onCreate }) => {
    ...
    <div className="fixed inset-0 bg-gradient-to-br from-black/60 to-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[95vh] overflow-hidden shadow-2xl">
        ...
        <div className="bg-gradient-to-r from-pink-500 to-rose-400 px-8 py-6 flex justify-between items-center">
          <h2 className="text-2xl font-bold text-white">Tạo Thử Thách</h2>
          <button
            onClick={onClose}
          ...
          <Formik
            ...
            onSubmit={async (values, { setSubmitting, resetForm }) => {
              ...
              try {
                ...
                console.error(err);
                toast.error("Lỗi khi tạo thử thách. Vui lòng thử lại.");
              } finally {
                setLoading(false);
                setSubmitting(false);
              }
            ...
            {({ values, setFieldValue, isSubmitting }) => (
                ...
                <div className="flex gap-6">
                  ...
                  <div className="w-1/2">
                    ...
                    <Field
                      type="date"
                      name="start_date"
                      min={calendarSetting.toISOString().split("T")[0]} // disables past dates
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                    />
                    <ErrorMessage
                      name="start_date"
                  ...
                  <div className="w-1/2">
                    ...
                    <Field
                      type="date"
                      name="end_date"
                      min={calendarSetting.toISOString().split("T")[0]} // also disallow past dates
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                    />
                    <ErrorMessage
                      name="end_date"
              ...
              hover:bg-pink-100 hover:border-pink-300"
                ...
                <div className="flex gap-6">
                  ...
                  <div className="w-1/2">
                    ...
                    <Field
                      as="textarea"
                      name="rules"
                      rows={4} // default height (you can increase to 6-8 if needed)
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {isSubmitting || loading ? "Đang tạo..." : "Tạo thử thách"}
                  </button>
                </div>
              </Form>
            )}
